...
import com.pdextended.pixeldungeonextended.ui.CheckBox;
...
public class WndSettings extends Window {
	...
	public WndSettings( boolean inGame ) {
		...
		if (inGame) {
			...
			CheckBox btnScaleUp = new CheckBox( TXT_SCALE_UP ) {
				...
				protected void onClick() {
					super.onClick();
					PixelDungeonExtended.scaleUp( checked() );
				}
			};
			btnScaleUp.setRect( 0, 0, WIDTH, BTN_HEIGHT );
			btnScaleUp.checked( PixelDungeonExtended.scaleUp() );
			add( btnScaleUp );
			...
			btnImmersive = new CheckBox( TXT_IMMERSIVE ) {
				...
				protected void onClick() {
					super.onClick();
					PixelDungeonExtended.immerse( checked() );
				}
			};
			btnImmersive.setRect( 0, btnScaleUp.bottom() + GAP, WIDTH, BTN_HEIGHT );
			btnImmersive.checked( PixelDungeonExtended.immersed() );
		...
		CheckBox btnSound = new CheckBox( TXT_SOUND ) {
			...
			protected void onClick() {
				super.onClick();
				PixelDungeonExtended.soundFx( checked() );
				Sample.INSTANCE.play( Assets.SND_CLICK );
			}
		};
		btnSound.setRect( 0, btnMusic.bottom() + GAP, WIDTH, BTN_HEIGHT );
		btnSound.checked( PixelDungeonExtended.soundFx() );
		add( btnSound );
		...
		if (inGame) {
			...
			CheckBox btnBrightness = new CheckBox( TXT_BRIGHTNESS ) {
				...
				protected void onClick() {
					super.onClick();
					PixelDungeonExtended.brightness( checked() );
				}
			};
			btnBrightness.setRect( 0, btnSound.bottom() + GAP, WIDTH, BTN_HEIGHT );
			btnBrightness.checked( PixelDungeonExtended.brightness() );
			add( btnBrightness );
			...
			RedButton btnOrientation = new RedButton( orientationText() ) {
				@Override
				protected void onClick() {
					PixelDungeonExtended.landscape( !PixelDungeonExtended.landscape() );
				}
			};
	...
	private void zoom( float value ) {
		...
		PixelDungeonExtended.zoom( (int)(value - PixelScene.defaultZoom) );
	...
	private String orientationText() {
		return PixelDungeonExtended.landscape() ? TXT_SWITCH_PORT : TXT_SWITCH_LAND;
	}
}
